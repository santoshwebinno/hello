...
class ModelExtensionDefaultUps extends Model {
	function getQuote($address) {
        ...
        $basic_products = $this->cart->getProducts();

#added by Stavros, start
		$quote_data = array();
		$shipping_surcharges = 0;
		$default_zip_code = $this->config->get('default_ups_postcode');
		$basic_cart_products = array( "$default_zip_code" => $basic_products);
		foreach($basic_cart_products["$default_zip_code"] as  $cart_product_key => $cart_product) {
			if($cart_product[shipping_price]) {
				$shipping_surcharges += $cart_product[shipping_price];
			}
			$cart_product[location] = preg_replace('/\D/', '', $cart_product[location]);
			if(preg_match('/^\d{5}$/',$cart_product[location]) && $cart_product[location]!=$default_zip_code) {
				if($basic_cart_products["$cart_product[location]"]) {
					$basic_cart_products["$cart_product[location]"][] = $cart_product;
				}
				else {
					$basic_cart_products["$cart_product[location]"] = array($cart_product);
				}
				unset($basic_cart_products["$default_zip_code"][$cart_product_key]);
			}
		}
		if(count($basic_cart_products["$default_zip_code"])==0) {
			unset($basic_cart_products["$default_zip_code"]);
		}
		...
		$mh = curl_multi_init();
		$handles = array();
		foreach($basic_cart_products as $key_zip => $value_products) {
			$ids_array = array();
			foreach($value_products as $c_product) {
				$ids_array[] = $c_product['product_id'];
			}
			$weight = $this->weight->convert($this->cart->getWeight($ids_array), $this->config->get('config_weight_class_id'), $this->config->get('default_ups_weight_class'));
			$weight = ($weight < 0.1 ? 0.1 : $weight);
			
			$request = $this->_buildRequest($address, $weight, $length, $width, $height, $key_zip);
			if($request) {
				$handles[] = $this->_processRequest( $request );
			    curl_multi_add_handle($mh, end($handles));
			}
			#if ($request) {
		...
		for($i = 0; $i < count($handles); $i++) 
		{
			...
			$new_quote_data = $this->_processRequestResult($out);
			$error_msg =  $new_quote_data['error_msg'];
            $new_quote_data =  $new_quote_data['quote_data'];
            foreach ($new_quote_data as $key => $value) {
				if($quote_data[$key]) {
					#if ($fixed_cost >= 0){
					...
					#}
					$quote_data[$key]['classid_count']++;	#added by Stavros
					...
					$quote_data[$key] = $new_quote_data[$key];
					$quote_data[$key]['classid_count'] = 1;
					$quote_data[$key]['cost'] += $shipping_surcharges;
					$quote_data[$key]['text'] = $this->currency->format(
																		$this->tax->calculate(
																							$quote_data[$key]['cost'],
																							$this->config->get('default_usps_tax_class_id'),
...
#added by Stavros,start
		...
		foreach($quote_data as $qdata_key => $qdata_val) {
			if($quote_data[$qdata_key]['classid_count']<count($basic_cart_products)) {
				unset($quote_data[$qdata_key]);
			}
		}
#end
    ...
    private function _buildRequest($address, $weight,$length,$width,$height,$keyzip){
        ...
        $xml .= '			<Address>';
        #$xml .= '				<City>' . $this->config->get('default_ups_city') . '</City>';
        #$xml .= '				<StateProvinceCode>'. ($statecode ? $statecode : $this->config->get('default_ups_state')) . '</StateProvinceCode>';
        $xml .= '				<CountryCode>' . $this->config->get('default_ups_country') . '</CountryCode>';
        $xml .= '				<PostalCode>' . ($keyzip ? $keyzip : $this->config->get('default_ups_postcode')) . '</PostalCode>';
        $xml .= '			</Address>';
        $xml .= '		</ShipFrom>';
...
private function _processRequestResult($result='') {
        $service_code = array(
            // US Origin
            'US' => array(
                ...
                foreach ($rated_shipments as $rated_shipment) {
                    ...
                    if (!$this->config->get('default_ups_' . strtolower($this->config->get('default_ups_origin')) . '_' . $code)) {
                        $quote_data[$code] = array(
                            ...
                            'text'         => $this->currency->format($this->tax->calculate($cost, $this->config->get('default_ups_tax_class_id'), $this->config->get('config_tax')))
                        );
                    }
                }
            }
